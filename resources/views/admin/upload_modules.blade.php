...
<body class="bg-white min-h-screen flex items-center justify-center">
  <div class="max-w-xl w-full p-5 space-y-5">
    ...
    <form class="space-y-6" method="POST" action="{{ route('modules.store') }}" enctype="multipart/form-data">
       @csrf
        <!-- Image Upload -->
<div>
...
@enderror
      <div>
        ...
        <input type="text" id="title" name="title" placeholder="Enter a file title"
...
@enderror
      ...
      <!-- Description -->
      <div>
        <label for="fileDescription" class="block text-sm font-medium text-black mb-1">Description</label>
        <textarea id="description" name="description" rows="4" placeholder="Add a brief description"
                  class="w-full px-2 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-black shadow-sm"></textarea>
      </div>
...
<div>
  ...
  <select id="subject_id" name="subject_id"
    ...
    @foreach ($subjects as $subject)
      <option value="{{ $subject->id }}" {{ old('course_id') == $subject->id ? 'selected' : '' }}>
        {{ $subject->title }}
      </option>
    @endforeach
  </select>
