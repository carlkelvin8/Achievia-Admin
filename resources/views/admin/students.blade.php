...
@include('admin.sidebar')
    ...
    <main class="flex-1 overflow-y-auto p-6">
      ...
      <section class="bg-white rounded-lg shadow p-6">
        <h2 class="text-2xl font-semibold text-gray-900 mb-4">RMT </h2>
          ...
          @foreach ($students as $student)
            <tbody class="bg-white divide-y divide-gray-200">
              <tr>
                ...
                <img src="{{ $student->profile_image ? asset('storage/' . $student->profile_image) : 'https://via.placeholder.com/40' }}"
     alt="Profile photo"
    ...
    {{ $student->section?->title ?? 'No Course' }}
</td>

                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{{ strtoupper($student->status) }}</span>
                </td>
                <td class="mt-10 text-right text-sm font-medium">
            <!-- Dropdown menu -->
            <div class="relative inline-block text-left">
    <!-- 3-dot icon button -->
    <button id="menu-button-{{ $student->id }}" type="button"
        ...
        onclick="document.getElementById('dropdown-{{ $student->id }}').classList.toggle('hidden')">
    ...
    <div id="dropdown-{{ $student->id }}"
        ...
        <div class="py-1">
            <a href="{{ route('students.show', $student->id) }}"
                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">View Student</a>
