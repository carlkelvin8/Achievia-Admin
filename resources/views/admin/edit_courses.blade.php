...
<body class="bg-white min-h-screen flex items-center justify-center">
  <div class="max-w-xl w-full p-5 space-y-5">
    ...
    <form class="space-y-6" method="POST" action="{{ route('courses.update', $course->id) }}" enctype="multipart/form-data">
       @csrf
       @METHOD('PUT')
<div>
...
@enderror
      <div>
        ...
        <input type="text" id="title" name="title" placeholder="Enter a file title" value="{{ old('title', $course->title) }}"
...
@enderror
  ...
  <textarea id="description" name="description" rows="4" placeholder="Add a brief description"
            class="w-full px-2 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-black shadow-sm">{{ old('description', $course->description) }}</textarea>
</div>
      @error('description')
    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
@enderror
