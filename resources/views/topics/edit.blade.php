  ...
  <div class="flex flex-1 overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto p-6">
      <section class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <h2 class="text-2xl font-semibold text-gray-900 mb-6">Edit Topic</h2>

        <form method="POST" action="{{ route('topics.update', $topic->id) }}" class="space-y-6">
          @csrf
          @method('PUT')
          ...
          <div>
            ...
            <textarea id="content" name="content" rows="8"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black tinymce">{{ old('content', $topic->content) }}</textarea>
            @error('content')
              <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
          </div>
